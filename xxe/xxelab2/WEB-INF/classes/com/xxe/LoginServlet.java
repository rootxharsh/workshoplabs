	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {		
		...
		try {
			...
			if(username.equals(USERNAME) && password.equals(PASSWORD)){
				result = String.format("<result><code>%d</code><msg>Success</msg></result>",1);
			}else{
				result = String.format("<result><code>%d</code><msg>Failure</msg></result>",0);
			}
		} catch (ParserConfigurationException e) {
			e.printStackTrace();
			result = String.format("<result><code>%d</code><msg>ParserConfigurationException</msg></result>",3);
		} catch (SAXException e) {
			e.printStackTrace();
			result = String.format("<result><code>%d</code><msg>SAXException</msg></result>",3);
		}
